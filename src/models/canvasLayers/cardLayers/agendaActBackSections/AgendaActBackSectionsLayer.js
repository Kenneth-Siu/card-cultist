    ...
    drawSection(currentY, header, story, text, useSeparator) {
        if (useSeparator && (header || story || text)) {
            ...
            this.context.fillRect(
                this.config.x,
                currentY + Math.round(this.config.fontSize * 0.7),
                this.config.width,
                1
            );
        ...
        if (header) {
            currentY = new CanvasTextLayer(
                this.headerConfig.withText(header).withY(currentY + this.config.fontSize * this.ACTUAL_LINE_HEIGHT)
            ).draw(this.context).y;
        }
        if (story) {
            if (header) {
            ...
            currentY = new CanvasTextLayer(
                this.storyConfig.withText(story).withY(currentY + this.config.fontSize * this.ACTUAL_LINE_HEIGHT)
            ).draw(this.context).y;

            this.context.save();
            this.context.fillStyle = "black";
            this.context.fillRect(this.config.x - 4, startY - 2, 2, currentY - startY + 4);
            this.context.fillRect(this.config.x + 2, startY - 2, 2, currentY - startY + 4);
            this.context.restore();
        }
        if (text) {
            if (header || story) {
            ...
            currentY = new CanvasTextLayer(
                this.textConfig.withText(text).withY(currentY + this.config.fontSize * this.ACTUAL_LINE_HEIGHT)
            ).draw(this.context).y;
        }
        return currentY;
    }
