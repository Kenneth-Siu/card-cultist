...
export default function MysticBackFaceCanvas({ face, cardSet, setIllustrationTransform }) {
    ...
    useEffect(() => {
        setTextLayer(
            new CanvasTextLayer(
                new CanvasTextConfig()
                    .withText(face.text)
                    .withX((dy) => dy < 330 ? Math.min(376 - 0.1 * dy, 610 - dy) : 44)
                    .withY(160)
                    .withWidth((dy) => dy < 330 ? Math.max(610 + 0.1 * dy, 376 + dy) : 942)
