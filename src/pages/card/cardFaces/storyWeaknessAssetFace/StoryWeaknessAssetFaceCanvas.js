...
import StoryWeaknessAssetFace from "./StoryWeaknessAssetFace";
...
import { weaknessIcons as skillIcons } from "../../../../../public/templates/skillIcons/skillIcons";
import skillIconFrame from "../../../../../public/templates/skillBoxes/weakness.png";
import { CampaignContext } from "../../../../components/CampaignContext";

export default function StoryWeaknessAssetFaceCanvas({ face, cardSet, setIllustrationTransform }) {
    ...
    useEffect(async () => {
        setFrameLayer(
            new CanvasImageLayer(await loadPublicImage(StoryWeaknessAssetFace.frame), new ImageTransform({ scale: 2 }))
        );
    }, []);
        ...
        setSubtitleFrameLayer(
            face.subtitle
                ? new CanvasImageLayer(
                      await loadPublicImage(StoryWeaknessAssetFace.subtitle),
            ...
            face.health
                ? new CanvasImageLayer(
                      await loadPublicImage(
                          face.health === "-" ? healthIcons[0] : healthIcons[Number.parseInt(face.health)]
                      ),
    ...
    useEffect(async () => {
        setSanityLayer(
            face.sanity
                ? new CanvasImageLayer(
                      await loadPublicImage(
                          face.sanity === "-" ? sanityIcons[0] : sanityIcons[Number.parseInt(face.sanity)]
                      ),
